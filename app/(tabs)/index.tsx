...
<<<<<<< HEAD
        ...
        const favoriteCount = Math.min(
          2,
          Math.max(1, Math.floor(favoriteContacts.length / 4))
        );

        const shuffledFavorites = [...favoriteContacts].sort(
          () => 0.5 - Math.random()
        );
        const favoritesToInclude = shuffledFavorites.slice(0, favoriteCount);

        const remainingContacts = shuffled
          .filter(c => !favoritesToInclude.some(fav => fav.id === c.id))
          .slice(0, 5 - favoritesToInclude.length);
